 ...
 * 我的打卡
 */
public class MyClockFragment extends BaseFragment{
    ...
    private void refreshData(){
        ...
        try {
            NetUtil.getResult(NetUtil.getUserClockUrl, params, getClockResult);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private class GetClockResult implements NetUtil.CallBackForResult {
        ...
        public void onFailure(final IOException e) {
            MyClockFragment.this.getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(MyClockFragment.this.getActivity(), e.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
        }
            ...
            if (response.isSuccessful()) {
                try {
                    ...
                    MyClockFragment.this.getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            adapter = new ClockListAdapter(MyClockFragment.this.getContext(), list);
